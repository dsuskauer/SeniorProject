...
function addActivity(newArr) {
    ...
    db.collection("Users").doc(username).collection("Activities").doc(date).set({
        ...
        document.write("You successfully Submitted Your Activities for Today")
}
...
//function to show the menu on current page of cafeteria menu
...
// Activity output from database
function getActivities() {
    var date = document.getElementById("searchUserActivities")[0].value;
    var desiredActivity=document.getElementById("searchUserActivities")[1].value;

    db.collection("Activities").doc(desiredActivity).collection(date).get().then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
